...
public class Controller {
            ...
            @RequestBody String eventsPayload)
    {
        try {
            ...
            //Reply for sound/picture/video message
            /*eventsModel.getEvents().forEach((event)->{
                if (event instanceof MessageEvent) {
                    if  ((  (MessageEvent) event).getMessage() instanceof AudioMessageContent
                        ...
                        String baseURL     = "https://traininglinejava.herokuapp.com";
            ...
            //Reply for group or one on one chat
            eventsModel.getEvents().forEach((event)->{
                if (event instanceof MessageEvent) {
    ...
    /*
    Message format for push message

    Format :
    http://appname.herokuapp.com/pushmessage/user_id/message

    Example :
    http://traininglinejava.herokuapp.com/pushmessage/U81e27e5c9de406f10c1e08a6e8687d77/Hello
    */
    ...
    /*
    Message format for multicast

    Format :
    http://appname.herokuapp.com/multicats

    Example :
    https://traininglinejava.herokuapp.com/multicast
    */
    @RequestMapping(value="/multicast", method=RequestMethod.GET)
    public ResponseEntity<String> multicast(){
        String[] userIdList = {
                "U81e27e5c9de406f10c1e08a6e8687d77"/*,
                "user_id_2",
                "user_id_3",
                "user_id_4",
                "user_id_5"*/};
        ...
        if(listUsers.size() > 0){
            String textMsg = "Hello everyone"; //Message Multicast
    ...
    /*
    Message format for profile with hardcode user id

    Format :
    http://appname.herokuapp.com/profile

    Example :
    https://traininglinejava.herokuapp.com/profile
    */
    @RequestMapping(value = "/profile", method = RequestMethod.GET)
    public ResponseEntity<String> profile(){
        String userId = "U81e27e5c9de406f10c1e08a6e8687d77"; //User ID
        UserProfileResponse profile = getProfile(userId);

        if (profile != null) {
            String profileName = profile.getDisplayName();
            TextMessage textMessage = new TextMessage("Hello, " + profileName); //Message
            PushMessage pushMessage = new PushMessage(userId, textMessage);
            push(pushMessage);
    ...
    /*
    Message format for profile with manual user id

    Format :
    http://appname.herokuapp.com/profile_manual/user_id

    Example :
    https://traininglinejava.herokuapp.com/profile_manual/U81e27e5c9de406f10c1e08a6e8687d77
    */
    @RequestMapping(value = "/profile_manual/{id}", method = RequestMethod.GET)
    public ResponseEntity<String> profileManual(
            @PathVariable("id") String userId
    ){
        UserProfileResponse profile = getProfile(userId);
